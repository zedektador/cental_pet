			...
			<?php 
				...
				for($cb=1;$cb<=$boxDisplay;$cb++){
					if(($cb >= $currentMonthFirstDay+1 || $currentMonthFirstDay == 7) && $cb <= ($totalDaysOfMonthDisplay)){
						...
						$result = $db->query("SELECT title FROM events WHERE date = '".$currentDate."' AND status = 0");
						...
						echo ($result)?'<a href="javascript:;" onclick="getEvents(\''.$currentDate.'\');">View Events</a><br/>':'';
...
function getEvents($date = ''){
	...
	$result = $db->query("SELECT title FROM events WHERE date = '".$date."' AND status = 0");
