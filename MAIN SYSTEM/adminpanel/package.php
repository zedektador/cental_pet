<?php 
	...
	if (isset($_POST['add'])) {
		$qry = "INSERT INTO tbl_admin_offered_packages (package_name, package_price, package_picture) VALUES ('$pname' , '$pprice' , '$ppicture')"; // inserting the data into the database
		...
		elseif (isset($_POST['button_edit'])) {
			$qry = mysqli_query($conn, "UPDATE tbl_admin_offered_packages SET package_name='".$_POST['package_name']."', package_price='".$_POST['package_price']."', package_picture='".$_FILES['picture']['name']."' WHERE id='".$_POST['id']."'"); // update data from database when 'edit button' is clicked
	...
	if (isset($_GET['delete'])) { 
		$qry = mysqli_query($conn, "DELETE FROM tbl_admin_offered_packages WHERE id='".$_GET['delete']."'"); // delete data from database when 'delete' is clicked
		if ($qry) {
	...
	elseif (isset($_GET['edit'])) {
		$qry = mysqli_query($conn, "SELECT * FROM tbl_admin_offered_packages WHERE id='".$_GET['edit']."'");
...
<body>
	...
	<!-- start: Header -->
		...
		<div class="row-fluid">
			...
			<form class="form-horizontal" action="package.php" method="POST" enctype="multipart/form-data">
			    ...
			    <input type="hidden" name="id" value="<?php echo $vid;?>">
			      <input type="text"  name="package_name" class="form-control" id="inputtext3" placeholder="Package Name" value="<?php echo $vname; ?>" required>
			    </div>
			     <label for="inputtext3" class="col-sm-2 control-label"><strong>Package Price:</strong></label>
			    <div class="col-sm-11">
			      <input type="text" name="package_price" class="form-control" id="inputtext3" placeholder="Package Price" value="<?php echo $vprice; ?>" required>
			    </div>
			     <div class="form-group">
			...
			<div class="row-fluid sortable">		
				<div class="box span12">
					<div class="box-header" data-original-title>
						<h2><i class="icon-align-justify"></i><span class="break"></span>Package Settings</h2>
						<div class="box-icon">
							    ...
							    <tbody>
							      <tr>
									...
									while ($row=mysqli_fetch_array($qry,MYSQLI_ASSOC)) {
										...
										echo '<td><a href="?edit='.$row['id'].'&package_picture='.$row["package_picture"].'"> Edit</a> | <a href="?view='.$row['id'].'"> View</a> | <a href="?delete='.$row['id'].'&picture='.$row["package_picture"].'"> Delete</td></tr>';
									}
								 ?>
							      </tr>
