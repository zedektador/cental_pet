...
session_start();
	...
	error_reporting(0);
					...
					if (isset($_GET['delete'])) { 
						$qry = mysqli_query($conn, "DELETE FROM tbl_customer_account WHERE id='".$_GET['delete']."'"); // delete data from database when 'delete' is clicked
							...
							<div class="container">       
							  <table class="table table-hover">
							    ...
							    <tbody>
							      <tr>
							        <?php 
							            $qry = mysqli_query($conn, "SELECT * FROM tbl_customer_account"); // query the data inside database
							            while ($row=mysqli_fetch_array($qry,MYSQLI_ASSOC)) {
							              echo '<tr><td>'.$row["customer_username"].'</td>';
							              ...
							              echo '<td>'.$row["customer_bday"].'</td>';


							             echo '<td>'.$row["customer_address"].'</td>';

							              echo '<td><a href="?delete='.$row['id'].'"> Delete</td></tr>';
							            }
							           ?>
							      </tr>
