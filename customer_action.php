...
if(isset($_POST["category"])){
	...
	echo "
				<div class='nav nav-pills nav-stacked'>
					<li class='active'><a href='customer_profile.php' style='background-color: #00b3b3;'><h4>Products </h4></a></li>
	";
//category sql
...
//brand sql
	...
	echo "
				<div class='nav nav-pills nav-stacked'>
					<li class='active'><a href='customer_profile.php' style='background-color: #00b3b3;'><h4>Services</h4></a></li>
	";
	if(mysqli_num_rows($run_query)>0){
		while($row=mysqli_fetch_array($run_query)){
			$bid = $row["brand_id"];
				...
				<div class='col-md-4'>
							<div class='panel panel-info'>
								...
								<div class='panel-body'>
									<img src='image/$pro_image' style='width:100%;' /><br><div class='title_pro'>$pro_desc</div><br><br> <b>Available Stock: $stocks</b>
...
if(isset($_POST["get_selected_Category"]) || isset($_POST["selectBrand"]) || isset($_POST["search"])){
			...
			echo "
				<div class='col-md-4'>
							<div class='panel panel-info'>
								<div class='panel-heading title_pro'>$pro_title</div>
								<div class='panel-body'>
									<img src='image/$pro_image' style= 'width:220px; height:250px;' /> <div class='title_pro'> $pro_desc </div> <br><br> <b>Available Stock: $qty</b>
								</div>
								<div class='panel-heading'>₱.$pro_price.00
								<button pid='$pro_id' style='float: right;' id = 'product' class='btn btn-danger btn-xs'>Add to Cart</button>
								</div>
	...
	if(isset($_POST["addProduct"])){
		...
		if($count > 0 ){
			...
			$sql = "INSERT INTO `cart` (`id`, `p_id`, `user_id`, `product_title`, `product_image`,`qty`, `price`, `total_amount`,`stocks`) VALUES (NULL, '$p_id', '$user_id', '$pro_name', '$pro_image','1', '$pro_price', '$pro_price',$stocks)";
 				...
 				elseif(isset($_POST["cart_checkout"])){
					...
					echo "
 						<div class='row'>
								...
								<div class='col-md-2'><input type='text' class='form-control quantity' pid='$pro_id' id='quantity-$pro_id' value='$qty' onchange ='quantity()' method = 'POST'></div>
 	...
 	if(isset($_POST["cart_checkout"])){
 		echo "
										...
										<div class='row'>
											...
											<div>
													<b>Total Amount : ₱ $total_amount.00</b>
											</div>
										</div> 
...
if(isset($_POST["cart_count"])){
	...
	echo mysqli_num_rows($run_query);
 	...
 	if(isset($_POST["updateProduct"])){
 		...
 		$price = $_POST["price"];
 		$total = $_POST["total"];
		$sqlv2 = "SELECT * FROM cart WHERE user_id = '$uid' AND p_id = '$pid' ";
		$queryv2 = mysqli_query($con , $sqlv2);
		$fetchv2 = mysqli_fetch_array($queryv2);
			$q = mysqli_query($con,"SELECT * FROM products where product_id = $pid ");
